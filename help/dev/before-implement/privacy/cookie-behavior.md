...
description: Informazioni su [!DNL Target] comportamento del cookie (cookie di prima parte, cookie di terze parti con cookie di prima parte o cookie di terze parti da solo).
...
>[!NOTE]
>
>Per informazioni dettagliate sui diversi cookie utilizzati da [!DNL Target], vedi [[!DNL Adobe Target] cookie](https://experienceleague.adobe.com/docs/core-services/interface/administration/ec-cookies/cookies-target.html?lang=it){target=_blank} nel *Guida ai componenti dell’interfaccia centrale di Experience Cloud*.
>
...
Vedi anche [Elimina [!DNL Target] cookie](cookie-deleting.md).

## Quando utilizzare un cookie di prima parte o di terze parti

Il cookie da utilizzare dipende dalla configurazione del sito. È utile capire come [!DNL Target] funziona quando si tenta di comprendere i cookie di prima parte e di terze parti. Consulta [Come [!DNL Adobe] [!DNL Target] Works](https://experienceleague.adobe.com/docs/target/using/introduction/how-target-works.html) per ulteriori informazioni.

I casi d’uso principali per i cookie sono tre:

1. Un dominio.

   Tutta l’attività di test si svolge all’interno di un dominio di primo livello (`www.domain.com`, `store.domain.com`, `anysub.domain.com`e così via).
...
1. Gli utenti passano da un dominio all’altro e tu intendi tenere traccia e sottoporre a test il loro comportamento tra i diversi domini.
   ...
   * Abilitare solo i cookie di prima parte e trasmettere il parametro `mboxSession` quando si passa a un altro dominio.
     ...
     Il `mboxSession` Il parametro deve essere passato a una pagina di destinazione e come riferimento dalla libreria JavaScript (Adobe Experience Platform Web SDK o at.js). Non può trattarsi di una pagina di reindirizzamento intermedia.
...
La libreria JavaScript genera un `mboxSession ID` e la memorizza nel [!DNL Target] cookie. La prima risposta mbox contiene l’offerta e il JavaScript per memorizzare l’ `mboxPC ID` generate dall’applicazione, nel cookie mbox.
...
Alcuni browser rifiutano i cookie di terze parti. Se il cookie di terze parti è bloccato, il cookie di prima parte continua a funzionare. [!DNL Target] tenta di impostare il cookie di terze parti e, se non è in grado di farlo, [!DNL Target] può solo tenere traccia del dominio specifico del cliente. Il tracciamento tra domini diversi non funziona se il cookie di terze parti è bloccato, a meno che `mboxSession` viene aggiunto al collegamento che attraversa più domini. In questo caso, un altro cookie di prima parte viene impostato e sincronizzato con il cookie di prima parte del dominio precedente.
...
| disable | Impostato se il tempo di caricamento del visitatore supera il timeout configurato nel file della libreria JavaScript. Per impostazione predefinita, la durata di questo valore è di un’ora. |

## Impatto su [!DNL Target] per i visitatori Safari a causa di modifiche nel monitoraggio di Apple WebKit

**In che modo [!DNL Target] lavoro di tracciamento?**
