...
description: Utilizza il [!UICONTROL adobe.target.applyOffers()] funzione per [!DNL Adobe Target] Libreria JavaScript at.js per applicare più offerte nella risposta. (at.js 2.x)
title: Come si utilizza [!UICONTROL adobe.target.applyOffers()] Funzione?
feature: at.js
...
| response > prefetch  > views > analytics > payload | [!DNL Analytics]Payload di per l’integrazione sul lato client da inviare ad dopo l’applicazione della visualizzazione.[!DNL Analytics] |
...
| response > execute > mboxes > mbox > analytics > payload | [!DNL Analytics]Payload di per l’integrazione sul lato client da inviare ad dopo l’applicazione della mbox. [!DNL Analytics] (Vedi la sezione Campagne abilitate per A4T.) |
...
| response > execute > pageLoad > analytics > payload | [!DNL Analytics]Payload di per l’integrazione sul lato client da inviare ad dopo l’applicazione del contenuto “pageLoad”. [!DNL Analytics] (Vedi la sezione Campagne abilitate per A4T.) |

## Esempio [!UICONTROL applyOffers()] chiamare
...
## Esempio di chiamate della catena di promesse con `[!UICONTROL getOffers()]` e `[!UICONTROL applyOffers()]`, perché queste funzioni si basano su Promesse
...
Per ulteriori esempi su come utilizzare getOffers(), consulta getOffers [documentazione](adobe-target-getoffers-atjs-2.md)
